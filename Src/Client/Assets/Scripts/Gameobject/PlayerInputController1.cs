...
public class PlayerInputController1 : MonoBehaviour {
    ...
    public EntiyController entityController;
    ...
    void FixedUpdate()
    {
        ...
        if (h<-0.1 || h>0.1)
        {
            ...
            if(rot.eulerAngles.y > this.turnAngle && rot.eulerAngles.y < (360 - this.turnAngle))//10度以内忽略不计
            {
    ...
    void SendEntityEvent(EntityEvent entityEvent)
    {
        if (entityController != null)
            entityController.OnEntityEnvent(entityEvent);
    }
}
