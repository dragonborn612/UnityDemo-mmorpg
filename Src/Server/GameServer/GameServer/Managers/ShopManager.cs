    ...
    class ShopManager:Singleton<ShopManager>
    {
        public Result BuyItem(NetConnection<NetSession> sender,int shopId ,int shopItemId)
        {
            ...
            if (DataManager.Instance.ShopItems[shopId].TryGetValue(shopItemId,out shopItem))
            {
                ...
                if (sender.Session.Character.Gold>=shopItem.Price)//如果角色的钱够
                {
                    sender.Session.Character.ItemManager.AddItem(shopItem.ItemID, shopItem.Count);//为当前会话角色添加物品 目前配表里的count为1
                    sender.Session.Character.Gold -= shopItem.Price;//扣钱
                    DBService.Instance.Save();//金币存入数据库
